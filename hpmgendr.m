...
%allocation for a resource constraint hybrid discrete time-cost trade-off
%problem (RCHDTCTP)
...
%---------------- 
%Usage:
%PSM=hpmgendr(PDM,const,typefcn)
%---------------- 
%Example:
...
%tic;PSM=hpmgendr(PDM,const,typefcn);toc
%---------------- 
...
function PSM=hpmgendr(PDM,const,typefcn)
...
options=gaoptimset('Display','off','PopulationSize',PopSize,...
    ...
    'HybridFcn',@fmincon,'CrossoverFcn',@crossoverhybriddr,...
...
rng(123)
chromosome=ga(@targetfcndr,n+2*N,[],[],[],[],LB,UB,@constrainrtpardr,...
    options);
